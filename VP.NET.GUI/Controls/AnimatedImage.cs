...
namespace VP.NET.GUI.Controls
{
    ...
    public class AnimatedImage : Image
    {
        ...
        private ANIHelper.AniFile? _aniFile;
        ...
        private void LoadAni(Stream stream)
        {
            stream.Seek(0, SeekOrigin.Begin);
            var ani = ANIHelper.ReadANI(stream);
            if (ani == null || ani.frames.Count == 0)
            {
                Log.Add(Log.LogSeverity.Error, "AnimatedImage.LoadAni", "Failed to parse ANI.");
                return;
            }

            _aniFile        = ani;
            _aniIndex       = 0;
            _reusableBitmap = ani.CreateMatchingBitmap();
            Source          = _reusableBitmap;
            _mode           = Mode.Ani;

            ani.frames[0].RenderTo(_reusableBitmap, ref _aniFile.header);
            InvalidateVisual();

            _lastDelayMs = ani.header.fps > 0 ? 1000 / ani.header.fps : 100;

            if (ani.frames.Count > 1 && Playing) StartTimer();
        }
        ...
        private void OnTick(object? sender, EventArgs e)
        {
            try
            {
                switch (_mode)
                {
                    ...
                    case Mode.Ani:
                        if (_aniFile != null && _reusableBitmap != null && _aniFile.frames.Count > 0)
                        {
                            _aniIndex = (_aniIndex + 1) % _aniFile.frames.Count;
                            _aniFile.frames[_aniIndex].RenderTo(_reusableBitmap, ref _aniFile.header);
                            InvalidateVisual();
                            int fps = _aniFile.header.fps;
                            _lastDelayMs = fps > 0 ? 1000 / fps : 100;
                        }
                        break;
